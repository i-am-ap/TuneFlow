...
# # --- Spotify imports only if credentials exist ---
# try:
#     from spotipy import Spotify
#     from spotipy.oauth2 import SpotifyClientCredentials
...
# if SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET and SPOTIFY_AVAILABLE:
#     sp = Spotify(auth_manager=SpotifyClientCredentials(
#         client_id=SPOTIFY_CLIENT_ID,
#         client_secret=SPOTIFY_CLIENT_SECRET
#     ))
# else:
#     sp = None
#     print("Warning: Spotify credentials not set, Spotify search disabled.")

# # --- Download paths ---
# BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# LOCAL_DOWNLOADS = os.path.join(BASE_DIR, 'downloads_local')
# PROD_DOWNLOADS = os.path.join(BASE_DIR, 'downloads_prod')

# if os.getenv('FLASK_ENV', 'production') == 'development':
#     SAVE_PATH = LOCAL_DOWNLOADS
# else:
#     SAVE_PATH = PROD_DOWNLOADS
...
# # --- Helper: YouTube search via yt-dlp ---
# def search_youtube(query: str) -> str:
...
#             title, artist, album, release_date = song, "", "", ""
...
#     job_id = str(uuid.uuid4())
#     JOBS[job_id] = {'state': 'queued', 'jobs': [{'name': s, 'status': 'queued', 'progress': 0, 'message': ''} for s in songs]}
#     threading.Thread(target=worker, args=(job_id, songs), daemon=True).start()
#     return jsonify({'job_id': job_id, 'initial': JOBS[job_id]['jobs']})
